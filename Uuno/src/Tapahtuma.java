...
public class Tapahtuma {
    ...
    public Tapahtuma(Teko tapahtuma, int pelaaja, Kortti[] kortit, Vari vari, int nostot) {
		this(tapahtuma, pelaaja, new Lyonti(new Vector<Kortti>(Arrays.asList(kortit)), vari), nostot);
    }

    // Tiedottaa pelaajalle tapahtumasta.
    public Tapahtuma(Teko tapahtuma, int pelaaja, Lyonti lyonti, 
            int nostot){
    	...
    	assert(tapahtuma != Teko.JAK || lyonti.getKortit().size()==1 && nostot==1);
    ...
    public String toString(){
       return("t:" + tapahtuma + " p:" + pelaaja + " l:" + lyonti.getKortit()
                 + " v:" + lyonti.getValittuVari() + " n:" + nostot);
    }
